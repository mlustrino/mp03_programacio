...
class JugadorCartas extends Jugador{
	...
	public JugadorCartas(String dni, String nombre, String apellidos, int numero_cartas,
			int mana, int vida) {
		super(dni, nombre, apellidos);
		this.numero_cartas = numero_cartas;
		this.mana = mana;
    ...
    public void iniciar_juego() {
        ...
        // while (!jugadores.ganador()) {
            ...
            while (jugadores_turno[0].getVida() >= 0 && jugadores_turno[1].getVida() >= 0) {
                ...
                acierto = carta_atacante.getAgilidad() >= numero_aleatorio;
                
                if (acierto) {
                    damage = carta_atacante.getAtaque() - (int) (carta_defensora.getDefensa() * carta_atacante.getAtaque() / 10);
...
class ContenedorJugadoresCartas {
    ...
    private final int NUMERO_JUGADORES = 11;
    private JugadorCartas[] jugadores = new JugadorCartas[NUMERO_JUGADORES];
    private int[] jugadoresEscogidos = new int[NUMERO_JUGADORES];
	...
	public void resetear_jugadores() {
        ...
        jugadoresEscogidos = new int[NUMERO_JUGADORES];
    }

    public void resetear_Jugadores_escogidos() {
        jugadoresEscogidos = new int[NUMERO_JUGADORES];
    }
    ...
    public JugadorCartas getJugadorAleotorio() {
        int numero_aleatorio;
        int numero_jugadores_escogidos = 0;
        if (!jugadores_completo()) {
            System.out.println("Primero mete todas las jugadores");
            return null;
        }
        for (int i = 0; i < NUMERO_JUGADORES; i++) {
            numero_jugadores_escogidos += jugadoresEscogidos[i];
        }
        if (numero_jugadores_escogidos == NUMERO_JUGADORES) {
            System.out.println("Ya no podemos coger mas jugadores");
            return null;
        }
        do {
            numero_aleatorio = (int) (NUMERO_JUGADORES * Math.random());
        } while (jugadoresEscogidos[numero_aleatorio] == 1);
        jugadoresEscogidos[numero_aleatorio] = 1;
...
class ContenedorCartas{
	...
	public Carta getCartaAleatoria() {
		...
		if (numero_cartas_escogidas == NUMERO_CARTAS) {
			System.out.println("Ya no podemos coger mas cartas");
			return null;
		}
		do {
